...
fdate=$(stat  -c %Y $CMK_FASTPINGER_DUMP)
date=
url="$@"
mode=$1
if [ "$mode" != "ping" ]; then
	...
	loss_crit=$(echo $crit | awk -F, '{gsub(/\..*/, "", $2); print $2}')
		...
		echo "$state - $dest : rta ${rt_display}ms, lost ${loss}% (from cmk_fastpinger at $fdate)|rta=${rt_display}ms;${rt_warn};${rt_cirt};0; pl=${loss}%;${loss_warn};${loss_crit};; rtmax=${rt_max}ms;;;; rtmin=${rt_min}ms;;;;"
                case $state in
                    OK) exit 0;;
                    WARNING) exit 1;;
                    CRITICAL) exit 2;;
